...
def test_login():
    ...
    body = html.parse('http://0.0.0.0:80').xpath('//body')[0].text_content()
    assert "Username" in body.strip()

    print("Connected!\n")
    print("Logging In: Username = brendy, Pass = flannle")
    loginInfo = {"username": "brendy", "password": "flannle"}
    req = requests.post('http://0.0.0.0:80/login', data=loginInfo, allow_redirects=True)
    ...
    req = requests.post('http://0.0.0.0:80/login', data=data, allow_redirects=True)
    while(req.status_code != 200):
        time.sleep(1)
